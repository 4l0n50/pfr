...
// ---------------------------------------------------------------------------
type PC<E> = MarlinKZG10<E, DensePolynomial<<E as PairingEngine>::Fr>>;
type Comm<E> = LabeledCommitment<
    <PC<E> as PolynomialCommitment<
        <E as PairingEngine>::Fr,
        DensePolynomial<<E as PairingEngine>::Fr>,
    >>::Commitment,
>;
type Rand<E> = <PC<E> as PolynomialCommitment<
    <E as PairingEngine>::Fr,
    DensePolynomial<<E as PairingEngine>::Fr>,
>>::Randomness;
type CK<E> = <PC<E> as PolynomialCommitment<
    <E as PairingEngine>::Fr,
    DensePolynomial<<E as PairingEngine>::Fr>,
>>::CommitterKey;
type VK<E> = <PC<E> as PolynomialCommitment<
    <E as PairingEngine>::Fr,
    DensePolynomial<<E as PairingEngine>::Fr>,
>>::VerifierKey;
...
pub(crate) fn round_three<E: PairingEngine>(
    ...
    debug_assert!(
        r_f.coeffs.get(0).map(|c| *c == E::Fr::zero()).unwrap_or(true),
...
pub fn prove<E: PairingEngine, R: RngCore>(
    ...
    let (round1_comms, round1_rands) =
        PC::<E>::commit(&pk.ck, round1_state.polynomials.iter(), None).unwrap();
...
mod tests {
    ...
    fn check_round2_sumcheck(pk: &PfrPublicKey<E>, row: &[usize], col: &[usize]) {
        let m = row.len();
        let r1 = round_one(pk, row, col, &mut ark_std::test_rng());
        let beta = F::from(42u64);
        let r2 = round_two(pk, &r1, beta, &mut ark_std::test_rng());
